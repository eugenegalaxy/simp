...
from simp.submodules.deepface.deepface.basemodels import VGGFace, OpenFace, Facenet, FbDeepFace, DeepID, DlibWrapper, ArcFace, Boosting
from simp.submodules.deepface.deepface.extendedmodels import Age, Gender, Race, Emotion
from simp.submodules.deepface.deepface.commons import functions, realtime, distance as dst
...
def verify(img1_path, img2_path='', model_name='VGG-Face', distance_metric='cosine', model=None, enforce_detection=True,
           detector_backend='mtcnn', dist_threshold=None, overwrite_cache=False):
    ...
    for index in pbar:
        ...
        if type(instance) == list and len(instance) >= 2:
            ...
            for i in model_names:
                ...
                if overwrite_cache is False:
                    if isinstance(img1_path, str):  # img_path:  ../../identity/coronapas.jpg
                        dirname = os.path.dirname(img1_path)  # ../../identity
                        orig_filename = os.path.splitext(os.path.basename(img1_path))[0]  # coronapas
                        cache_filename = '{0}_{1}.txt'.format(model_name, orig_filename)  # VGG-Face_coronapas.txt
                        full_pt_cache = os.path.join(dirname, cache_filename)  # ../../identity/VGG-Face_coronapas.txt
                        if os.path.isfile(full_pt_cache):
                            try:
                                with open(full_pt_cache, 'r') as file:
                                    img1_representation = file.read().split(',')
                                    img1_representation[0] = img1_representation[0].replace('[', '')
                                    img1_representation[-1] = img1_representation[-1].replace(']', '')
                                    img1_representation = list(img1_representation)
                                    img1_representation = [float(x) for x in img1_representation]
                        ...
                        overwrite_cache = True  # if img_path is not a string path, ignore looking for cached txt embedding and just do it.

                    if isinstance(img2_path, str):  # img_path:  ../../identity/coronapas.jpg
                        dirname = os.path.dirname(img2_path)  # ../../identity
                        orig_filename = os.path.splitext(os.path.basename(img2_path))[0]  # coronapas
                        cache_filename = '{0}_{1}.txt'.format(model_name, orig_filename)  # VGG-Face_coronapas.txt
                        full_pt_cache = os.path.join(dirname, cache_filename)  # ../../identity/VGG-Face_coronapas.txt
                        if os.path.isfile(full_pt_cache):
                            try:
                                with open(full_pt_cache, 'r') as file:
                                    img2_representation = file.read().split(',')
                                    img2_representation[0] = img2_representation[0].replace('[', '')
                                    img2_representation[-1] = img2_representation[-1].replace(']', '')
                                    img2_representation = list(img2_representation)
                                    img2_representation = [float(x) for x in img2_representation]
                            except Exception as e:
                                print(e)
                                overwrite_cache = True
                        else:
                        ...
                        # if img_path is not a string path, ignore looking for cached txt embedding and just do it.
                        overwrite_cache = True
                ...
                for j in metrics:
                    ...
                    if model_name != 'Ensemble':
                        ...
                        if dist_threshold is None:
                            ...
                            threshold = dist_threshold  # I hope you know what you are doing!
...
def represent(img_path, model_name = 'VGG-Face', model = None, enforce_detection = True, detector_backend = 'mtcnn'):
    ...
    if isinstance(img_path, str):  # img_path:  ../../identity/coronapas.jpg
        dirname = os.path.dirname(img_path)  # ../../identity
        orig_filename = os.path.splitext(os.path.basename(img_path))[0]  # coronapas
        cache_filename = '{0}_{1}.txt'.format(model_name, orig_filename)  # VGG-Face_coronapas.txt
        full_pt_cache = os.path.join(dirname, cache_filename)  # ../../identity/VGG-Face_coronapas.txt
        with open(full_pt_cache, 'w') as file:
