...
TEST_NAME = 'mask_wrong'
REL_PT = 'yolo_dataset/mask_wrong'
            ...
            if ext == '.jpg' or ext == '.jpeg' or ext == '.png':
                ...
                if finding is not None:
                    ...
                    if generate_images is True:
                        save_mask_pt = os.path.join(detections_pt, '{0}_{1}_{2}.jpg'.format(
                            idx, finding["guess"], finding["conf"]))
                        cv2.imwrite(save_mask_pt, bb_img)
                else:
                    DEBUG('Image {0} -> No detections , File Name: {1}'.format(img_file, finding["file_name"]))
    return all_findings


LOG.write('================================================================================================')
LOG.write('Starting Test: Accuracy of "{}" detection ============================================='.format(TEST_NAME))

LOG.write('')
LOG.write('Test: Accuracy of Really A Robot YOLOv5(small) Mask Detector on detecting a class "{}'.format(TEST_NAME))
LOG.write('Description: In this test, only the images with ground truth "{}" are giving to the system.'.format(
    TEST_NAME))
...
LOG.write('[{0}/{1}] ({3}%) Label {2} was guessed {0} times correctly out {1} total images with {2} ground-truth.'.format(
    total_cnt_correct, num_of_scans, TEST_NAME, ((total_cnt_correct / num_of_scans) * 100)))
